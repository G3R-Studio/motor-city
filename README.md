...
- an FCG decor palette under `Motor City > Fantastic City Generator > 6 - Open Decor Palette` for manually placing authored city props inside `City-Maker`: bus stops, benches, hydrants, dumpsters, trash cans, cones, lamps, signs, parks/flowerbeds/fountain and several parked vehicle prefabs; placed props remain ordinary scene objects and are included by the normal runtime city build;
