...
- additional PolyPack cars use a rebuilt runtime physics integration rather than the starter car's fixed geometry: wheel radius/centers are measured from each model, a shorter adaptive suspension is configured per wheel size, the chassis collider is fitted to the visible body, and a post-Prometeo wheel visual synchronizer drives suspension travel, steering and wheel rotation directly from the four WheelColliders;
