...
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City uses the prefab's day/night skyboxes, ambient colors and sun/moon colors; FCG street-light anchors receive dedicated realtime Point Lights, nearby lamps are enabled only at night, Forward+ rendering is used to keep dense street lighting stable on large city meshes, and lamp discovery is cached instead of rescanning the whole scene during gameplay;
...
- the authored Fantastic City Generator map is the single source of truth: `Build Runtime City from Saved FCG City` copies the saved `City-Maker` into `CityVisual.prefab` without automatically changing colliders, props, parked vehicles, traffic signals, lights or other map content, and the runtime then treats that prefab as read-only; gameplay systems only read the finished city geometry for spawn points, navigation, routes and vehicle reset placement;
- runtime open-world traffic built on top of the authored FCG road geometry: up to 16 AI cars spawn outside the immediate camera view around the player, follow detected road surfaces, choose junction turns, slow for obstacles and the player, obey a basic two-axis traffic-light cycle, despawn when far away and automatically use the starter car plus locally generated PolyPack vehicle visuals when those resources are available;
