...
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City uses the prefab's day/night skyboxes, ambient colors and sun/moon colors, while the original FCG lamp objects named `Spot Light` / `_Spot_Light` are preserved as realtime lights and enabled only at night; unrelated city materials are never globally brightened;
