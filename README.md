...
- a smooth orbiting chase camera with mouse look, zoom, speed-based look-ahead, distance and field-of-view response; the camera remains non-physical but uses sphere-cast obstacle avoidance so it pulls in along buildings and solid city geometry instead of clipping through them;
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City uses the prefab's day/night skyboxes, ambient colors and sun/moon colors; every leaf lamp anchor named `Spot Light` / `_Spot_Light` receives a dedicated downward realtime Spot Light, nearby lamps are enabled only at night, URP additional-light support is enabled when available, and lamp discovery is cached instead of rescanning the whole scene during gameplay;
...
- FCG traffic lights run on a coordinated runtime cycle: nearby signal heads are grouped into intersections, opposing directions share one phase, crossing directions stay red, green transitions through yellow with a short all-red clearance, and pedestrian indicators switch opposite the vehicle phase; legacy FCG traffic-light animations are disabled so only the Motor City controller owns signal state;
- city collision coverage is repaired during runtime-city baking and again when the city loads: architectural meshes under FCG building hierarchies receive static MeshColliders where missing, existing building colliders are re-enabled, and a hidden safety floor just below street level prevents the player car from falling through small geometry/collision gaps; road/highway physics is rebuilt at runtime from upward-facing road-material-only triangles, legacy highway colliders are disabled first, and road materials are excluded from broad building-collider repair so combined highway sides, decorations or guardrail geometry do not become invisible ramps;
