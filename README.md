...
- the authored Fantastic City Generator map is treated as read-only at runtime: Motor City does not add, remove, re-enable or disable map colliders, does not delete parked vehicles or street props, and does not alter traffic-signal objects after the city is loaded; collision and object setup come directly from the baked `CityVisual.prefab`;
