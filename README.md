...
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City uses the prefab's day/night skyboxes, ambient colors and sun/moon colors, while FCG lamp nodes named `Spot Light` / `_Spot_Light` preserve their existing Light components (including child Lights) or receive a realtime fallback Spot Light when missing; lamp discovery is cached once when the city starts instead of rescanning the whole scene during gameplay, and the lamps are enabled only at night;
