...
- Prometeo acceleration, steering, service braking, coasting and handbrake traction-loss behavior, with Motor City garage upgrades mapped onto the controller tuning; when steering input is released, a post-Prometeo straight-line assist recenters the front WheelColliders and damps small unintended lateral/yaw drift without fighting an active slide; the player can switch a persistent three-mode driving profile at runtime: Comfort for stable everyday driving, Sport for maximum acceleration/speed and high rear-axle traction, and Drift with a 70-degree steering lock, more progressive rear grip, softer traction break and a counter-steer recovery assist that only intervenes when the driver actively steers against the slide;
