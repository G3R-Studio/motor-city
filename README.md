...
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City uses the prefab's day/night skyboxes, ambient colors and sun/moon colors, while the actual FCG lamp `Spot Light` / `_Spot_Light` components provide night street illumination; unrelated city materials are not globally brightened at night;
- editor tooling for **Fantastic City Generator**: generated FCG renderers in the active scene can be converted from legacy/Built-in materials to URP/Lit while preserving base textures, normal maps, occlusion, emission and transparent/cutout behavior; duplicate FCG material names are repaired by their source GUID so separate foliage material slots keep the correct texture; the generated `City-Maker` can then be baked into a local runtime `CityVisual.prefab`;
...
- breakable street furniture is baked into the generated city: individual street lights, park lamps, traffic lights, hydrants, trash bins, benches, poles and recognized road signs use dedicated non-blocking trigger colliders plus a swept PlayerCar chassis sensor for reliable high-speed impacts; original FCG prop colliders stay disabled, traffic lights topple around their actual base, broken props remain visible for several seconds and only then fade away, and lamp spot lights switch off permanently when their pole is knocked down;
