...
- a runtime day/night cycle built from the local FCG DayNight prefab: Motor City extracts the prefab's day/night skyboxes, ambient colors, sun/moon colors and day/night material pairs into an ignored runtime settings asset, rotates sun/moon lighting through the cycle, swaps the FCG city to its night material variants and enables preserved point/spot city lights only at night;
...
- breakable street furniture is baked into the generated city: individual street lights, park lamps, traffic lights, hydrants, trash bins, benches, poles and recognized road signs use tight non-blocking trigger colliders, react only to the PlayerCar root chassis rather than wheel/imported visual colliders, take only a small amount of vehicle speed on impact, topple around their ground contact instead of vanishing, remain visible for several seconds after falling and only then fade away; FCG street/park lamp lights turn on only at night and switch off permanently when the pole is knocked down;
